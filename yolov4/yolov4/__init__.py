...
def create_modules(module_defs, img_size, cfg):
    ...
    for i, mdef in enumerate(module_defs):
        ...
        if mdef["type"] == "convolutional":
            ...
            layers = mdef["layers"]
            filters = sum(
                [output_filters[l + 1 if l > 0 else l] for l in layers]
            )
            routs.extend([i + l if l < 0 else l for l in layers])
            ...
            layers = mdef["layers"]
            filters = (
                sum([output_filters[l + 1 if l > 0 else l] for l in layers])
                // 2
            )
            routs.extend([i + l if l < 0 else l for l in layers])
            ...
            if any(
                x == cfg.stem for x in ["yolov4-tiny", "fpn", "yolov3"]
            ):  # P5, P4, P3 strides
                stride = [32, 16, 8]
...
class Darknet(nn.Module):
    ...
    def __init__(self, cfg=None, img_size=(416, 416), verbose=False):
        ...
        self.info(
            verbose
        ) if not ONNX_EXPORT else None  # print model description
    ...
    def forward_once(self, x, augment=False, verbose=False):
        ...
        for i, module in enumerate(self.module_list):
            ...
            if verbose:
                print(
                    "%g/%g %s -" % (i, len(self.module_list), name),
                    list(x.shape),
                    str,
                )
                str = ""
        ...
        for i, (mdef, module) in enumerate(
            ...
            if mdef["type"] == "convolutional":
                ...
                conv.weight.data.copy_(
                    torch.from_numpy(weights[ptr : ptr + nw]).view_as(
                        conv.weight
                    )
                )
                ptr += nw
...
class YOLOLayer(nn.Module):
    def __init__(self, anchors, nc, img_size, yolo_index, layers, stride):
        ...
        self.nx, self.ny, self.ng = (
            ...
            0,
            0,
        )  # initialize number of x, y gridpoints
    ...
    def forward(self, p, out):
        ...
        if self.training:
            ...
            anchor_wh = (
                self.anchor_wh.repeat(1, 1, self.nx, self.ny, 1).view(m, 2) * ng
            )
...
class JDELayer(nn.Module):
    ...
    def forward(self, p, out):
        ...
        if self.training:
            ...
            io = p.clone()  # inference output
            io[..., :2] = (
                torch.sigmoid(io[..., :2]) * 2.0 - 0.5 + self.grid
            )  # xy
            io[..., 2:4] = (
