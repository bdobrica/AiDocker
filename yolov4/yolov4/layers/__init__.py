...
class FeatureConcat2(nn.Module):
    ...
    def forward(self, x, outputs):
        return torch.cat(
            [outputs[self.layers[0]], outputs[self.layers[1]].detach()], 1
        )
    ...
    def forward(self, x, outputs):
        return (
            ...
            if self.multiple
                ...
                :, : outputs[self.layers[0]].shape[1] // 2, :, :
            ]
        )
...
class MixConv2d(
    ...
    def __init__(
        ...
        if method == "equal_ch":  # equal channels per group
            i = torch.linspace(
                0, groups - 1e-6, out_ch
            ).floor()  # out_ch indices
