...
class AIDaemon(Daemon):
    def ai(self, source_file, prepared_file, **metadata):
        ...
        try:
            ...
            MODEL_PATH = os.environ.get(
                "MODEL_PATH", "/opt/app/facenet_keras.h5"
            )
            ...
            if faces is not None:
                ...
                db_files = Path(
                    DATABASE_PATH, metadata.get("partition", "default")
                ).glob("*.npy")
                face_ids = []
                face_distances = None
                for db_file in db_files:
                    ...
                    cos_distance = (
                        np.matmul(signatures, face_cmp.T)
                        / np.linalg.norm(signatures, axis=1)
                        / np.linalg.norm(face_cmp, axis=1)
                    ).max(axis=1)
                ...
                if face_distances is not None:
                    ...
                    results = {
                        "signatures": base64_signatures,
                        "similarities": [
                            {"id": "unknown", "box": box, "similarity": 0}
                            for box in boxes
                        ],
                    }
                ...
                for result in results:
                    ...
                    text_size = cv2.getTextSize(
                        label, 0, fontScale=0.5, thickness=1
                    )[0]
                    cv2.rectangle(
                        img_copy,
                        (int(box_left), int(box_top)),
                        (
                            int(box_left + text_size[0]),
                            int(box_top - text_size[1] - 3),
                        ),
                        color,
                        -1,
                    )
                ...
                results = results = {"signatures": "", "similarities": []}
    ...
    def check_db(self):
        ...
        if time.time() - latest_load < float(
            os.environ.get("DATABASE_CHECK_INTERVAL", 60)
        ):
            return
        ...
        query = faces_table.select().where(
            faces_table.c.updated_at > latest_date
        )
