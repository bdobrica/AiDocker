    ...
    def ai(self, source_file, prepared_file, **metadata):
        ...
        try:
            ...
            MODEL_PATH = os.environ.get(
                "MODEL_PATH", "/opt/app/modnet_photographic_portrait_matting.onnx"
            )
            ...
            if background.startswith("http://") or background.startswith("https://"):
                try:
                    ...
                    background_im = cv2.resize(
                        background_im, dsize=(im_w, im_h), interpolation=cv2.INTER_AREA
                    )
            ...
            alpha_mask = np.repeat(
                out_im[:, :, 3].reshape(out_im.shape[:2] + (1,)) / 255.0, 3, axis=2
            )
